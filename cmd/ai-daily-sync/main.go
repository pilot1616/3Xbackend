...
func main() {
	...
	syncService := service.NewAIDailySyncService(db.Connect, cfg.Sync.AIDaily)
	if err := syncService.SyncOnce(context.Background()); err != nil {
		log.Fatalf("sync ai daily failed: %v", err)
	}

	log.Printf("ai daily sync finished successfully")
}
