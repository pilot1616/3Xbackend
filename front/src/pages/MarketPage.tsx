    ...
    return [
      {
        ...
        tone: (strongest?.numericChangePercent ?? 0) > 0 ? 'up' : (strongest?.numericChangePercent ?? 0) < 0 ? 'down' : 'flat',
      },
      {
        label: '价格最高',
      ...
      <div className="cont w1000 market-shell">
        <div className="market-route-banner">
          ...
          <div className="market-route-copy">
            ...
            <span>{marketType === 'precious-metals' ? '聚焦贵金属价格、区间和合约信息。' : '聚焦 AI / 科技相关指数与 ETF 的价格轨迹。'}</span>
          </div>
              ...
              <div className="market-panel market-symbol-panel">
                ...
                <div className="market-symbol-list">
                  {visibleRecords.map((record) => (
                    <button className={`market-symbol-card${record.symbol === activeRecord?.symbol ? ' is-active' : ''}`} key={record.symbol} onClick={() => setActiveSymbol(record.symbol)} type="button">
                  ...
                  <div className="market-grid">
                    <section className="market-panel market-kpi-panel">
                      ...
                      <div className="market-kpi-grid">
                        <article className="market-kpi-card"><span>前收</span><strong>{activeRecord.prevClose || '--'}</strong></article>
                        <article className="market-kpi-card"><span>开盘</span><strong>{activeRecord.open || '--'}</strong></article>
                        <article className="market-kpi-card"><span>买价 / 卖价</span><strong>{activeRecord.bid || '--'} / {activeRecord.ask || '--'}</strong></article>
                        <article className="market-kpi-card"><span>日内区间</span><strong>{activeRecord.dayRange || '--'}</strong></article>
                        <article className="market-kpi-card"><span>52 周区间</span><strong>{activeRecord.week52Range || '--'}</strong></article>
                        <article className="market-kpi-card"><span>成交量 / 均量</span><strong>{activeRecord.volume || '--'} / {activeRecord.avgVolume || '--'}</strong></article>
                      </div>
                    </section>
